...
| 新訓練線 | `TRACKED-LINEAGE-TRAINING-V1` 於 2026-09-14 **執行完成**：scratch、5 replicates、20 個 checkpoint 進版控（`38.0 MiB`）、10 次執行皆 `RUN_LOCK_BOUND`。**`PUB-B1` 達成**（provenance 可重建的訓練線存在）；**`PUB-B2` 未達成**——5 個 replicate 的 full exposure 皆 `0/30`，標籤 `TL_REFERENCE_NOT_ATTAINED`。**這是事先宣告的結果，門檻不得因此下調** | [receipt](docs/TRACKED_LINEAGE_TRAINING_RECEIPT_2026-09-14.md) |
...
| [TRACKED_LINEAGE_TRAINING_RECEIPT_2026-09-14](docs/TRACKED_LINEAGE_TRAINING_RECEIPT_2026-09-14.md) | 執行結果：`PUB-B1` 達成、`PUB-B2` `NOT_ATTAINED`（`0/5` replicate 達標）、失敗型態量測與 claim boundary |
