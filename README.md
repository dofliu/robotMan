...
| V&V gates | V0 PARTIAL、V1 PARTIAL、V2–V4 NOT STARTED。V1 於 2026-09-22 補上 known pendulum 閉式解與被動 articulated 能量／收斂（6/6 PASS，[receipt](docs/receipts/V1_DYNAMIC_REFERENCE_SUITE_RECEIPT_2026-09-22.md)），並補上單剛體 dynamic contact（drop-and-settle、黏滯滑塊 7/7 PASS；預登記的 Coulomb 假說被拒絕——plant 滑動摩擦在 ≳ 0.2 m/s 不是 Coulomb，[receipt](docs/receipts/V1_CONTACT_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；仍缺關節式人形的 dynamic contact、致動能量帳、solver／finite-difference、joint limits、actuator envelope | [PROJECT_STATUS §1](docs/PROJECT_STATUS.md) |
...
| 下一個決策 | **Track B formal evaluation 於 2026-09-22 決定延後**（負責人委託採納建議）：若執行則先預註冊替代規則；`20000–20029` 兩條線都不花、保持封存；解除條件是有訓練線達到 `PUB-B2` 出口條件。**現在唯一在等負責人的是 Track A**：`PUB-A0` 剩餘 `U` 條目的原文 PDF | [TRACK_B_FORMAL_EVALUATION_DECISION](docs/TRACK_B_FORMAL_EVALUATION_DECISION_2026-09-22.md)、[PUBLICATION_PLAN §5](docs/PUBLICATION_PLAN.md) |
...
| [V1_CONTACT_REFERENCE_SUITE_SPEC](docs/V1_CONTACT_REFERENCE_SUITE_SPEC.md) | **V1 接觸參考案例的凍結規格**——自由球 drop-and-settle、平移滑塊黏滯滑動、預登記的 Coulomb 假說，全用 plant 的接觸參數；§1.1 揭露設計期觀察（自由薄板會翹起跳離、滑動摩擦沒有 Coulomb 區間） |
...
| 測試 | `backend/` **1 failed / 1113 passed**（2026-09-22，`550.82` s，1,114 收集，`c4cd809` 的乾淨工作樹（凍結 commit；其後的 `59eab53` 只有文件、無程式變更），`python3 -X utf8 -m pytest backend/ -p no:cacheprovider`；比同日 `64ad5d6` 的 1,092 多的 22 個全在 `test_v1_contact_reference_suite.py`）。另有 **`frontend/e2e` 6 項瀏覽器測試**（2026-09-22 全過，不在 backend 集合裡，[receipt](docs/receipts/BROWSER_VISUAL_VERIFICATION_RECEIPT_2026-09-22.md)）。那一個失敗是在具名 environment lock 下**記錄為量測結果、未放寬**的 reduction-order 差異——看到它不必修 | [PROJECT_STATUS §9](docs/PROJECT_STATUS.md)、`STATUS.yaml` 的 `test_suite_status` |
...
| [`docs/receipts/`](docs/receipts/README.md) | **實作 receipt 索引**——17 份（14 份於 2026-09-20 由 `docs/` 搬入，加 2026-09-22 的瀏覽器視覺驗證、V1 動態參考案例、V1 接觸參考案例三份 receipt）；仍然有效，只是不是規劃文件 |
