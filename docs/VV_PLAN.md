...
| V1-R11 numerical convergence | time-step, solver tolerance and finite-difference study | metric/residual convergence under preregistered criterion | convergence table + raw runs | V1 | PARTIAL: passive 4/2/1 ms normalized-GRF stability PASS，observed order依 frozen round-off semantics為 null；**2026-09-22 加** known pendulum 週期（observed order 2.0）與 articulated 被動擺動能量殘差（order 0.99）兩個 4/2/1 ms 動態收斂研究，皆 PASS（[receipt](receipts/V1_DYNAMIC_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；**同日再加** 黏滯滑動對連續閉式的一階收斂（order 1.15／1.07）與穩態穿透的 dt 無關性（極差 ≤ 2e-8，[receipt](receipts/V1_CONTACT_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；solver-tolerance 與 finite-difference 仍缺 |
...
| V1-R13 energy consistency | power balance and physics-step integration | independent recomputation agreement | tau/omega/drive-loss trace | V1 | PARTIAL（2026-09-22，原 BLOCKED）: 被動 articulated 的能量平衡含關節阻尼功可由序列化量以 stdlib 獨立重算，殘差 0.70／0.35／0.17%（4/2/1 ms）在凍結門檻內，引擎能量記帳一致到 1e-15（[receipt](receipts/V1_DYNAMIC_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；**致動下的 tau／omega／drive-loss 帳仍缺** |
