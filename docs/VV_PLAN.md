...
| V1-R05 unilateral contact | inspect each contact normal force | all active Fz nonnegative within tolerance | contact trace + violation list | V1 | PARTIAL: non-adhesive double/single-support minimum normal force gates與 replay PASS；dynamic scenario coverage仍缺 |
| V1-R06 friction feasibility | friction cone/pyramid constraint | tangential force inside frozen friction model | friction params + per-contact ratios | V1 | PARTIAL: frozen PYRAMIDAL/`condim=3` double/single-support utilization與 replay PASS；dynamic scenarios仍缺 |
