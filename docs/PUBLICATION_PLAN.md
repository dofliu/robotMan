...
| `PUB-A2` Claim freeze | [TRACK_A_REFRAME §5](TRACK_A_REFRAME_2026-09-09.md) 每列可只由 hash-bound receipts 推出；§6 不可宣稱清單與 §7 figure／table 清單凍結；Limitations 先寫 | `IN_PROGRESS` — 草稿即 TRACK_A_REFRAME；2026-09-11 另凍結 [`R0-REGIME-HORIZON-PROBE-V1`](R0_REGIME_PROBE_SPEC.md)，用於把 taxonomy 的 `R0` 空格改為有依據的陳述（pilot，結果未產生） | 凍結須在 A0 之後；R0 probe 的結果（含 null 結果）須先寫入 §3 |
...
2. **`PUB-A2`**：`PUB-A1a` 已 PASS、`PUB-A1b` 已關閉；下一步是在 A0 之後把 [TRACK_A_REFRAME §5–§7](TRACK_A_REFRAME_2026-09-09.md) 凍結為 claim freeze receipt。同時核對 rl-zoo recipe 數值（影響限制清單第 5 條的措辭）。**不再開任何第二案例 probe 或 protocol**；2026-09-11 凍結的 [`R0-REGIME-HORIZON-PROBE-V1`](R0_REGIME_PROBE_SPEC.md) 不是第二案例線——它不訓練、不評估、不動 seed，只對既有 450 個 retained episode 做唯讀重算，用於補 taxonomy 的 `R0` 格。
