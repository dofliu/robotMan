...
| `PUB-B1` Tracked training line | 新線每個 checkpoint 與 warm start 進版控或 immutable storage；每個 run 以 `RUN-MANIFEST-LOCK-BINDING-V1` 綁定 lock record（經 `backend/rl/bind_run_lock.py`，否則 gate 判為 `RUN_LOCK_UNBOUND`） | `PROTOCOL_FROZEN_NOT_EXECUTED` — [TRACKED-LINEAGE-TRAINING-V1](TRACKED_LINEAGE_TRAINING_SPEC.md) 於 2026-09-14 凍結：scratch（不得 warm start）、5 replicates、`GIT_DIRECT` 儲存每 `500,000` 步共 `20` 個 checkpoint、`TL-CK-01`..`TL-CK-05`。綁定機制已於 2026-09-13 就緒。**尚未執行** |
| `PUB-B2` Reliable-completion baseline | reference policy 在 DEV seeds 上達到**事先凍結**的 full-exposure 比例 | `THRESHOLD_FROZEN_NOT_EXECUTED` — 門檻由專案負責人於 2026-09-14、**在看到任何訓練曲線之前**定為 `30/30 = 1.000000`，逐 replicate 在 DEV seeds `22000–22029` 上判定（[spec §4.2](TRACKED_LINEAGE_TRAINING_SPEC.md)）。凍結後**不得因結果下調**；未達即 `NOT_ATTAINED`，那是一項結果 |
| `PUB-B3` Pilot variance → N | point-valued between-replicate SD；N 由 power analysis 決定並寫進 preregistration；不得事後上調 | `BLOCKED` by B2，且**另需一份 protocol**：`TRACKED-LINEAGE-TRAINING-V1` 明示不產生 `control_step_trace`，取得它須修改 `backend/rl/eval_policy.py`，而那會弄紅綁在 owner 已授權 protocol 上的綠測試（[spec §6.4](TRACKED_LINEAGE_TRAINING_SPEC.md)） |
