...
Early-termination / exposure-censoring validity audit V1 已在 frozen bundle 上完成，並量測確認 V7C 的 `-36.2185185` pp 是 exposure artifact。接續的 `SEEDVAR-V7-TRAINING-REPLICATE-DEV-V1` 已凍結：5 個 independent fine-tuning training seeds `8720/8740/8760/8780/8800`（12-env blocks 兩兩不重疊且與 pilot 的 `8700–8711` 不重疊）、共用 DEV evaluation seeds `18000–18029`、每個 run 前必須 verify `MEASURED_ENVIRONMENT_LOCK` + `FULL_LOCK` + `AMBIENT_THREADING_PINNED`。它**已執行完成**：`1,843,200` realized timesteps、450 個 terminal records、0 失敗。V7B 相對 V7A 的 method-level bound 為 `[-13.503408, -12.435259]` pp（排除 0、sign NEGATIVE、5/5 replicates 可識別），但 `between_replicate_sd` 因每個 replicate 都有 exposure censoring 而為 `null`，故 sample-size 決策仍 blocked；V7C 為 `[-37.195407, +27.315704]` pp 含 0、0/5 可識別。Independent pretraining-seed variance 已查證不可量測（v5 的 warm start 來源不在版本控制內），selection protocol 已凍結但不可執行。formal authorization 已於 2026-09-10 取得，但 frozen protocol JSON 內 `EP-03` 仍為 `BLOCKING`、`PUB-B4` 外部預註冊仍在解封之前，且量測顯示 `SEL-C2` 在此線上幾乎確定不成立（[PUB_B0_AUTHORIZATION_RECEIPT](PUB_B0_AUTHORIZATION_RECEIPT_2026-09-10.md)）。仍不調 envelope/filter/threshold、不存取 FORMAL/HOLDOUT，也不得回溯調整 `replicate_count`。Multi-speed policy、turn、terrain與其他 primitive仍需另立 profile/protocol version，不可改 label冒充。v2以前的完整數值見 [START_STOP_POLICY_TRAINING_RECEIPT_2026-08-30](START_STOP_POLICY_TRAINING_RECEIPT_2026-08-30.md)。
