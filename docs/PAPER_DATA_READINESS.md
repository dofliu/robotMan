...
| PDR-2 Run identity | manifest schema、source/model/controller/environment hashes | IN PROGRESS / content-sensitive pre/post Git、exact MJCF/model package與 clean-source identity PASS；v7另綁定 protocol/profile/checkpoint/evaluation/source identities；`ENVIRONMENT-LOCK-V1`已能量測與重驗 environment identity並保留一份實測 record，但沒有任何 pipeline把 lock record綁進 run manifest，且 v7 retained bundles為 `ABSENT_UNRECOVERABLE` |
...
14. [NEXT] 決定 [PUBLICATION_PLAN §5](PUBLICATION_PLAN.md) 的兩個子問題（用現行 post-hoc 規則或先預註冊替代規則；唯一未檢視的 FORMAL 範圍花在 v7 線或保留給 `PUB-B1`／`PUB-B2` 的新訓練線），再依序完成 `PUB-B4`、`EP-03` narrowing amendment、`EP-01`／`EP-02` amendment；在那之前不做 selection、不調 threshold、不鑄造機器可讀的 authorization evidence、不存取 `20000–20029`。
