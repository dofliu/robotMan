...
10. [DONE-SOFTWARE / BLOCKED-COMPUTE] 凍結 [TRAINING_SEED_VARIANCE_SPEC](TRAINING_SEED_VARIANCE_SPEC.md) 與 `SEEDVAR-V7-TRAINING-REPLICATE-DEV-V1`，並完成其 evidence contract、獨立 `python -I -S` replay 與 101 個 fail-closed tests。Analysis unit 固定在 **training replicate**：method-level 分母恆為 `5`，`150`／`450` 為 enforced forbidden denominators（`PSEUDO_REPLICATION_FORBIDDEN`）。Exposure censoring 逐層組合為 tight intervals，任一 replicate difference 不可點識別即封鎖 `between_replicate_sd`（sample SD 沒有定義在 interval 上）；含 method failure 的 cell 沒有 mean。Selection 永久禁止、`replicate_count` 不得事後上調。三個 synthetic case 全部 replay exact。**尚未執行任何訓練**（需 `3 × 5 × 122,880 = 1,843,200` timesteps），因此沒有 v7 method-level variance 數值，`method_level_power_ready=false`、`formal_sample_size_decision=BLOCKED_UNTIL_THIS_PROTOCOL_EXECUTES`。詳見 [seed-variance receipt](TRAINING_SEED_VARIANCE_IMPLEMENTATION_RECEIPT_2026-09-08.md)。
...
12. [NEXT] 執行已凍結的 seed-variance protocol，並把 lock record 綁進每一條 pipeline 的 run manifest。在 seed-variance 結果出現前不做 selection、不調 threshold、不開啟 FORMAL/HOLDOUT；`replicate_count` 不得因算力不足而減少。
