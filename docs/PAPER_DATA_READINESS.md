...
13. [DONE-DECISION] formal authorization 已於 2026-09-10 取得（[PUB_B0_AUTHORIZATION_RECEIPT](PUB_B0_AUTHORIZATION_RECEIPT_2026-09-10.md)）。[BLOCKER] 那是一項決定、不是狀態變更：frozen protocol JSON 內 `EP-03` 仍硬寫 `BLOCKING`（`assert_executable()` 讀的是該 JSON），`EP-01`／`EP-02` 未動，且凍結順序為「授權在前、預註冊在中、解封在後」而 `PUB-B4` 只有專案負責人能做。[RESULT] 同一份 receipt 量測出 `SEL-C2` 在 v7 線上幾乎確定不成立：reference `V7A` 自己只有 143/150 episode `COMPARABLE`、`V7B` 120/150、`V7C` 0/150，聯合通過機率為 `2.2 × 10⁻¹⁸`／`0`。
14. [DONE-FORWARD-ONLY] 凍結 [RUN_MANIFEST_LOCK_BINDING_SPEC](RUN_MANIFEST_LOCK_BINDING_SPEC.md) 的 `RUN-MANIFEST-LOCK-BINDING-V1` 並實作：`RUN_LOCK_BINDING_V1` sidecar 記錄以 SHA-256 釘住其 run manifest，`PAPER_RUN_MANIFEST` 升至 `V2` 另內嵌同一份 lock block，五個 fail-closed 標籤（`RUN_LOCK_BOUND`／`UNBOUND`／`INSUFFICIENT`／`MISMATCH`／`BINDING_METHOD_FAILURE`，最後一個永不降級），gate 要求 `MEASURED` **且** `FULL_LOCK` 而非只看 `lock_class`。`LB-01`..`LB-12` 共 62 個測試通過。[RESULT] 量到一個活著的缺陷：同名欄位 `environment_lock_sha256` 在兩份保留 bundle 裡分別是 lock 檔案位元組與 `locked` 子樹 digest，而兩次執行的 `locked_sha256` 相同——新記錄以兩個名字保留兩個量並永久禁用該模糊名稱。[BLOCKER] 前向、未清除：sidecar 可被遺漏（直接呼叫 driver 得 `RUN_LOCK_UNBOUND`）、`backend/simulator.py` 明示排除、2026-09-08 bundle 未重建。詳見 [binding receipt](RUN_MANIFEST_LOCK_BINDING_RECEIPT_2026-09-13.md)。
