...
12. [DONE-CLOSED / DONE-SOFTWARE] 兩件事：(a) independent **pretraining**-seed variance 經查證**不可量測**——v5 warm start 自 gitignored `backend/rl/artifacts/` 下的 v4 local artifact、版本控制只有 3 個 policy artifact、v5 profile 記 `warm_start_policy_id: null`、且其 checkpoint 是在另一個 regressed run 之後被選出的，因此 `CONDITIONAL_ON_FIXED_WARM_START` 對 v7 line 永久成立（見 [infeasibility receipt](V7_PRETRAINING_SEED_VARIANCE_INFEASIBILITY_RECEIPT_2026-09-08.md)）；(b) 凍結 [V7_CANDIDATE_SELECTION_SPEC](V7_CANDIDATE_SELECTION_SPEC.md) 的 `SELECT-V7-CANDIDATE-FORMAL-V1` 與其 fail-closed gate，`preregistered=false` 由 contract 強制，只能在 sealed FORMAL 上決策、只套用一次、eligible 需六個明確 PASS。[RESULT] 規則套在已看過的 DEV evidence 上選不出 candidate。[BLOCKER] 不可執行：`EP-01` audit contract 拒絕 `SEALED_SEED_RANGE` seeds、`EP-02` eval driver 釘死 seed schedule、`EP-03` 無 formal authorization。詳見 [selection receipt](V7_CANDIDATE_SELECTION_IMPLEMENTATION_RECEIPT_2026-09-08.md)。
