...
| PDR-2 Run identity | manifest schema、source/model/controller/environment hashes | IN PROGRESS / content-sensitive pre/post Git、exact MJCF/model package與 clean-source identity PASS；v7另綁定 protocol/profile/checkpoint/evaluation/source identities；`ENVIRONMENT-LOCK-V1`已能量測與重驗 environment identity並保留一份實測 record，`RUN-MANIFEST-LOCK-BINDING-V1`（2026-09-13）以 SHA-256 把 lock record綁進 run manifest並提供 fail-closed gate，但該綁定是**前向**的，v7 retained bundles仍為 `ABSENT_UNRECOVERABLE` |
...
| PDR-5 Independent metrics | raw-only evaluator覆蓋 primary/secondary outcomes | PARTIAL / static、analytical、v7 action/raw-summary、exposure audit與 seed-variance summary皆有 stdlib-only exact replay，且 frozen v7 bundle的 audit run已完成；seed-variance replay僅在 synthetic rows上驗證（未執行訓練），Study A outcomes未覆蓋 |
| PDR-6 Statistics | paired effects、CI、binary intervals、censoring | SOFTWARE CONTRACT PARTIAL / continuous paired mean、median、Cohen dz與 deterministic bootstrap CI已實作；exposure-censoring audit另加入 method-failure與 censoring分離、assumption-free full-horizon/paired identification bounds與 blocked aggregate；`SEEDVAR-V7-TRAINING-REPLICATE-DEV-V1`另把 analysis unit固定在 training replicate（method-level分母恆為 replicate 數，episode-level分母為 enforced forbidden denominators）並把 censoring向上組合為 replicate/method-level intervals，partial identification直接封鎖 between-replicate SD；該 protocol已執行完成，V7B的 method-level bound排除0且5/5 replicates sign-identified，但 between-replicate SD因 exposure censoring仍為 null，故 sample-size決策 blocked；binary paired CI與 actual Study A仍 blocked |
...
[RESULT] [V7 exposure-censoring audit receipt](archive/V7_EXPOSURE_CENSORING_AUDIT_IMPLEMENTATION_RECEIPT_2026-09-08.md)在 clean Git
...
- Methods：phase-observable PPO、joint-specific action envelope、action rate/low-pass filtering；v7尚未選出 eligible candidate，exposure-censoring audit已完成、獨立 training-seed protocol已凍結但未執行；WBC residual 等 P1/P2完成後再加入。
...
- Sample size：先用 PILOT variance/power analysis決定，不在看到 formal results 後調整。所需的 between-replicate variance由 `SEEDVAR-V7-TRAINING-REPLICATE-DEV-V1`提供；該 protocol的 `replicate_count`本身亦不得在看到結果後上調（optional stopping），且其 SD因共用固定 v5 warm start而系統性低估完整 method variance，任何 sample-size計算必須標註此低估方向。

目前 v2–v7 runs 都是 DEVELOPMENT evidence；不得直接重新標記為 formal replicates。v7的30個 evaluation seeds不能替代獨立 training replicates —— 這條規則現在由 `SEEDVAR-V7-TRAINING-REPLICATE-DEV-V1`以 enforced forbidden denominators在程式層強制執行，而不只是文件敘述。
...
1. [DONE] 完成 `PAPER_RUN_MANIFEST_V1` 與 artifact validator（2026-09-13 升至 `V2`，見第 14 項；`V1` 仍可讀，既有 bundle 不失效）。
...
9. [DONE] 對 `V7_PILOT_DEVELOPMENT_BUNDLE`完成一次 read-only audit run，`audit_applies_to_frozen_v7_pilot=true`、`AX-01..AX-12`全通過、read-only前後 readback一致、保留 35個 censoring blocker。實測 exposure為 V7A 30/30 full（450 steps）、V7B 27 full + 3 early（420–445 steps，全落在 `FINAL_STAND`且 required outcomes仍 `OBSERVED`）、V7C 30/30 early（`159.7000 ± 2.7687` steps，`3.08–3.30` s，占 horizon `0.354889`，全落在 `STEADY_WALK`）。V7C full-horizon bound `[0.0, 64.511111]`%與 V7A `36.2185185`%重疊，paired bound `[-36.2185185, +28.2925927]`含0、`0/30` sign-identified；V7B paired bound `30/30`排除0且皆 NEGATIVE，但 aggregate仍 `NULL`。詳見 [frozen bundle receipt](archive/V7_EXPOSURE_CENSORING_AUDIT_FROZEN_BUNDLE_RECEIPT_2026-09-08.md)。
10. [DONE] 凍結並**執行完成** [TRAINING_SEED_VARIANCE_SPEC](archive/TRAINING_SEED_VARIANCE_SPEC.md) 的 `SEEDVAR-V7-TRAINING-REPLICATE-DEV-V1`。Analysis unit 固定在 **training replicate**（method-level 分母恆為 `5`，`150`／`450` 為 enforced forbidden denominators），exposure censoring 逐層組合為 tight intervals，含 method failure 的 cell 沒有 mean，selection 永久禁止。實際執行：`1,843,200` realized timesteps、450 個 terminal records、0 失敗、獨立 `python -I -S` replay exact，證據保留於 `backend/seed_variance_evidence/2026-09-08/`。[RESULT] V7B 相對 V7A 的 method-level bound 為 `[-13.503408, -12.435259]` pp（排除 0、sign NEGATIVE、5/5 replicates 可識別），但 `between_replicate_sd` 因每個 replicate 都有 exposure censoring 而為 `null`，故 sample-size 決策仍 blocked；V7C 為 `[-37.195407, +27.315704]` pp 含 0、0/5 可識別；450 個 episodes 全為 `COMPARABLE`（263）或 `EXPOSURE_CENSORED`（187）、零 method failure；pilot 那個乾淨的 reference exposure 被量測確認是 seed `8700` 的性質而非 V7A 的性質。[BLOCKER] `sample_size_decision_input_ready=false`、`method_level_power_ready=false`、`selected_candidate_arm_id=null`；5 個 replicate 共用同一個 v5 warm start，量到的變異系統性低估完整 method variance。詳見 [execution receipt](archive/TRAINING_SEED_VARIANCE_EXECUTION_RECEIPT_2026-09-08.md) 與 [implementation receipt](archive/TRAINING_SEED_VARIANCE_IMPLEMENTATION_RECEIPT_2026-09-08.md)。
11. [DONE-SOFTWARE / BLOCKED-BINDING] 凍結 [ENVIRONMENT_LOCK_SPEC](ENVIRONMENT_LOCK_SPEC.md) 並實作 `ENVIRONMENT-LOCK-V1`：digested `locked` 與 retained-but-undigested `observed` 分界、behaviour fingerprints（實跑 `500` 步 MuJoCo contact state digest、實際執行的 torch optimiser step、frozen pure-Python LCG 輸入）、typed `UNSET`／`ABSENT_UNRECOVERABLE` 狀態與 stdlib-only digest replay，`EL-01..EL-10` 共 60 個測試通過。實測到同一組 1000 個 reciprocal 在同一環境下依 stdlib 順序得 `7.485470860550343`、經 `numpy.ndarray.sum` 得 `7.485470860550345`，兩者皆符合 IEEE 754 —— 這是「pin 版本號不足以重驗數值」的直接證據。詳見 [environment lock receipt](receipts/ENVIRONMENT_LOCK_IMPLEMENTATION_RECEIPT_2026-09-08.md)。綁定部分見第 14 項。
12. [DONE-CLOSED / DONE-SOFTWARE] 兩件事：(a) independent **pretraining**-seed variance 經查證**不可量測**——v5 warm start 自 gitignored `backend/rl/artifacts/` 下的 v4 local artifact、版本控制只有 3 個 policy artifact、v5 profile 記 `warm_start_policy_id: null`、且其 checkpoint 是在另一個 regressed run 之後被選出的，因此 `CONDITIONAL_ON_FIXED_WARM_START` 對 v7 line 永久成立（見 [infeasibility receipt](archive/V7_PRETRAINING_SEED_VARIANCE_INFEASIBILITY_RECEIPT_2026-09-08.md)）；(b) 凍結 [V7_CANDIDATE_SELECTION_SPEC](archive/V7_CANDIDATE_SELECTION_SPEC.md) 的 `SELECT-V7-CANDIDATE-FORMAL-V1` 與其 fail-closed gate，`preregistered=false` 由 contract 強制，只能在 sealed FORMAL 上決策、只套用一次、eligible 需六個明確 PASS。[RESULT] 規則套在已看過的 DEV evidence 上選不出 candidate。[BLOCKER] 不可執行：`EP-01` audit contract 拒絕 `SEALED_SEED_RANGE` seeds、`EP-02` eval driver 釘死 seed schedule、`EP-03` 無 formal authorization。詳見 [selection receipt](archive/V7_CANDIDATE_SELECTION_IMPLEMENTATION_RECEIPT_2026-09-08.md)。
13. [DONE-DECISION] formal authorization 已於 2026-09-10 取得（[PUB_B0_AUTHORIZATION_RECEIPT](receipts/PUB_B0_AUTHORIZATION_RECEIPT_2026-09-10.md)）。[BLOCKER] 那是一項決定、不是狀態變更：frozen protocol JSON 內 `EP-03` 仍硬寫 `BLOCKING`（`assert_executable()` 讀的是該 JSON），`EP-01`／`EP-02` 未動，且凍結順序為「授權在前、預註冊在中、解封在後」而 `PUB-B4` 只有專案負責人能做。[RESULT] 同一份 receipt 量測出 `SEL-C2` 在 v7 線上幾乎確定不成立：reference `V7A` 自己只有 143/150 episode `COMPARABLE`、`V7B` 120/150、`V7C` 0/150，聯合通過機率為 `2.2 × 10⁻¹⁸`／`0`。
14. [DONE-FORWARD-ONLY] 凍結 [RUN_MANIFEST_LOCK_BINDING_SPEC](RUN_MANIFEST_LOCK_BINDING_SPEC.md) 的 `RUN-MANIFEST-LOCK-BINDING-V1` 並實作：`RUN_LOCK_BINDING_V1` sidecar 記錄以 SHA-256 釘住其 run manifest，`PAPER_RUN_MANIFEST` 升至 `V2` 另內嵌同一份 lock block，五個 fail-closed 標籤（`RUN_LOCK_BOUND`／`UNBOUND`／`INSUFFICIENT`／`MISMATCH`／`BINDING_METHOD_FAILURE`，最後一個永不降級），gate 要求 `MEASURED` **且** `FULL_LOCK` 而非只看 `lock_class`。`LB-01`..`LB-12` 共 62 個測試通過。[RESULT] 量到一個活著的缺陷：同名欄位 `environment_lock_sha256` 在兩份保留 bundle 裡分別是 lock 檔案位元組與 `locked` 子樹 digest，而兩次執行的 `locked_sha256` 相同——新記錄以兩個名字保留兩個量並永久禁用該模糊名稱。[BLOCKER] 前向、未清除：sidecar 可被遺漏（直接呼叫 driver 得 `RUN_LOCK_UNBOUND`）、`backend/simulator.py` 明示排除、2026-09-08 bundle 未重建。詳見 [binding receipt](receipts/RUN_MANIFEST_LOCK_BINDING_RECEIPT_2026-09-13.md)。
15. [NEXT] 決定 [PUBLICATION_PLAN §5](PUBLICATION_PLAN.md) 的兩個子問題（用現行 post-hoc 規則或先預註冊替代規則；唯一未檢視的 FORMAL 範圍花在 v7 線或保留給 `PUB-B1`／`PUB-B2` 的新訓練線），再依序完成 `PUB-B4`、`EP-03` narrowing amendment、`EP-01`／`EP-02` amendment；在那之前不做 selection、不調 threshold、不鑄造機器可讀的 authorization evidence、不存取 `20000–20029`。
