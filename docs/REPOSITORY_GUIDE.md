...
- `docs/assets/`：文件引用的圖與數據，含 [TEST_REPORT_2026-09-20](TEST_REPORT_2026-09-20.md) 的五張 UI 截圖，以及 [CONTROLLER_COMPARISON_2026-09-22](CONTROLLER_COMPARISON_2026-09-22.md) 的 7 張圖、`summary.json` 與 50 Hz CSV（由 `backend/compare_controllers_report.py` 產生）、[RAIBERT_STACK_DIAGNOSIS_2026-09-22](RAIBERT_STACK_DIAGNOSIS_2026-09-22.md) 的 2 張圖與 CSV（`backend/diagnose_raibert_stack.py`）、[瀏覽器視覺驗證 receipt](receipts/BROWSER_VISUAL_VERIFICATION_RECEIPT_2026-09-22.md) 的五張截圖（`ui-2026-09-22/`）。
- `frontend/e2e/`（**2026-09-22 建立**）：五個畫面的 dedicated 瀏覽器測試（pytest + Playwright，6 項），跑在 production 路徑；`python3 -X utf8 -m pytest frontend/e2e -p no:cacheprovider -q`。**不在 `pytest backend/` 的集合裡**（需要 Node、Chromium 與可綁定的埠）；缺任何一項都是失敗而非 skip。截圖寫到 `frontend/e2e/artifacts/`（gitignored）。
