...
- **`PUB-B0` 已授權（2026-09-10）**：專案負責人授權 formal evaluation（[PUB_B0_AUTHORIZATION_RECEIPT](receipts/PUB_B0_AUTHORIZATION_RECEIPT_2026-09-10.md)）。授權**不使 protocol 可執行**：frozen protocol JSON 內 `EP-03` 仍硬寫 `BLOCKING`、`EP-01`／`EP-02` 未解除，且凍結順序要求 `PUB-B4` 外部預註冊在解封 sealed seeds `20000–20029` 之前，而預註冊只有專案負責人能做。當日**未存取** FORMAL seeds。
...
| V1 Plant & Numerical | `PARTIAL_IMPLEMENTED_NOT_PASS` | static double-support V4 16/14 exact；analytical fixture（passive single-support、centered 5 kg payload、4/2/1 ms grid）4/4 PASS，含 raw Jacobian stdlib-only replay；**2026-09-22**：known pendulum 閉式週期／能量 4/2/1 ms 3/3 PASS（order 2.0），articulated 被動擺動能量平衡（含阻尼功、閉式球／盒慣量）3/3 PASS（order 0.99），stdlib replay 相符（[receipt](receipts/V1_DYNAMIC_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；**同日** contact reference suite：自由球 drop-and-settle 3/3 與平移滑塊黏滯滑動 3/3 PASS（觸地步逐字符合離散閉式、衝量對重量積分 2e-12、平衡穿透對引擎模型閉式 2e-8），stdlib replay 210 個 metric 相符，**預登記的 Coulomb 假說三條 rejected**（[receipt](receipts/V1_CONTACT_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；**同日** actuated energy suite：開環正弦扭矩（無接觸）與 plant 站立 PD 蹲起（含接觸功）能量帳 6/6 PASS，殘差 0.18／0.084／0.041% 與 0.23／0.13／0.069%，replay 144 個 metric 相符（[receipt](receipts/V1_ACTUATED_ENERGY_SUITE_RECEIPT_2026-09-22.md)） | 關節式人形的 dynamic contact（步態的 unilateral／friction／CoP／schedule；單剛體部分已於 2026-09-22 完成）、plant 滑動摩擦在 ≳ 0.2 m/s 非 Coulomb 的已知特性（承認或改參數重測）、行走／撞擊情境的能量帳（致動能量帳的無接觸與貼地慢動作版已完成；plant 無 drive-loss 模型）、solver-tolerance／finite-difference 研究、joint limits、actuator envelope；已有的 articulated 案例是被動、無接觸的（2026-09-22 更正：原列「articulated dynamic、known pendulum、energy balance」的被動版已完成） |
...
| 項目 | 結果 | 性質 |
|---|---|---|
| V1 執行（2 arms × 5 replicates × 301,056 steps） | 300 episodes、284 EARLY／16 FULL；reference 4/5 replicates 30/30 早跌；naive `W2D_C − W2D_A` `−28.795138` pp、t-interval `[−46.698919, −10.891357]` 排除 0；θ `[−79.118, +55.913333]` 含 0、0/5；`SECOND_CASE_ARTIFACT_REPRODUCED`；P3 284/284 `OBSERVED` | DEVELOPMENT，凍結 protocol，replay exact（[receipt](archive/SECOND_CASE_EXPOSURE_CENSORING_EXECUTION_RECEIPT_2026-09-08.md)） |
...
[BLOCKER] Probe 的凍結 claim boundary 只支持 budget 選擇；不得用於任何關於 Walker2d／Hopper／PPO recipe 能力的陳述。2026-09-09 起 `…-WALKER2D-V2`／`…-HOPPER-V1` 兩個 protocol id 撤回（從未 pin、從未載入），詳見 [probe receipt §7](archive/SECOND_CASE_V2_BUDGET_PROBE_RECEIPT_2026-09-08.md)。
...
- V1 仍缺關節式人形的 dynamic contact、行走／撞擊情境的能量帳、solver-tolerance／finite-difference 研究、joint limits、actuator envelope；known pendulum 與被動 articulated 的能量／收斂已於 2026-09-22 補上（[receipt](receipts/V1_DYNAMIC_REFERENCE_SUITE_RECEIPT_2026-09-22.md)），單剛體的 dynamic contact（drop-and-settle、黏滯滑塊）同日補上並量到 **plant 的滑動摩擦在 ≳ 0.2 m/s 不是 Coulomb**（法向力膨脹 3 倍、失去接觸，[receipt](receipts/V1_CONTACT_REFERENCE_SUITE_RECEIPT_2026-09-22.md)）；致動能量帳（開環正弦與 plant 站立 PD 蹲起，含接觸功）同日完成，殘差 ≤ 0.23%、一階收斂，plant 無 drive-loss 模型這點明列為限制（[receipt](receipts/V1_ACTUATED_ENERGY_SUITE_RECEIPT_2026-09-22.md)）；V2 全缺。任何 controller 結論的 plant 可信度尚未建立。（2026-09-22 更正：原文「V1 缺 articulated dynamic、pendulum、energy、solver convergence」）

### 6.5 Authorization 與 preregistration

- `EP-03` formal authorization **決定已於 2026-09-10 取得**（[receipt](receipts/PUB_B0_AUTHORIZATION_RECEIPT_2026-09-10.md)），但 frozen protocol JSON 內該項仍硬寫 `BLOCKING`，而 `assert_executable()` 讀的是該 JSON；改為 `RESOLVED` 需要一次 narrowing-only、execution-before 的 amendment 並重新 pin contract 內的 `PROTOCOL_SHA256`。
...
- UI：`development_compare_mode` 與 `dynamic_run_trace` 的 `BROWSER_VISUAL_PENDING` **已於 2026-09-22 解除**（`frontend/e2e` 6 項瀏覽器測試全過，[receipt](receipts/BROWSER_VISUAL_VERIFICATION_RECEIPT_2026-09-22.md)，`PUB-C0` PASS）；**frontend 仍不獨立驗證 server config hash**（未變）。（2026-09-22 更正：原文「`development_compare_mode` 與 `dynamic_run_trace` 皆 `BROWSER_VISUAL_PENDING`」——其餘敘述保留：frontend 不獨立驗證 server config hash。2026-09-16 的介面改版**沒有改變這兩點**——它只動版面與呈現，證據語義、API 與 token 集合都沒動。）
...
| 2026-09-22 | `V1-CONTACT-REFERENCE-SUITE-V1`：自由球 drop-and-settle 與平移滑塊黏滯滑動，凍結後執行；預登記 Coulomb 假說（[receipt](receipts/V1_CONTACT_REFERENCE_SUITE_RECEIPT_2026-09-22.md)） | 7/7 gate PASS 首輪即過、replay 210 個 metric 相符；H1／H2／H3 rejected——plant 滑動摩擦在 0.5 m/s 法向力膨脹 3.03 倍、22 ms 失去接觸；`V1-R08` NOT STARTED → PARTIAL；V1 gate 仍 `PARTIAL_IMPLEMENTED_NOT_PASS` |
| 2026-09-22 | `V1-ACTUATED-ENERGY-SUITE-V1`：致動能量帳——開環正弦扭矩（軀幹固定）與 plant 站立 PD 蹲起（含接觸功），凍結後執行（[receipt](receipts/V1_ACTUATED_ENERGY_SUITE_RECEIPT_2026-09-22.md)） | 6/6 PASS 首輪即過、replay 144 個 metric 相符；殘差 0.18／0.084／0.041% 與 0.23／0.13／0.069%；`V1-R13` 致動部分完成、drive loss 明列為 plant 限制；發現 `implicitfast` 有阻尼時不是半隱式 Euler（隱式修正 4e-3～1e-2）；V1 gate 仍 `PARTIAL_IMPLEMENTED_NOT_PASS` |
...
2. `PUB-A2` claim freeze：A0 之後，把 [TRACK_A_REFRAME §5–§7](TRACK_A_REFRAME_2026-09-09.md) 凍結。**不再開任何第二案例 probe 或 protocol**；`PUB-A1b` 已關閉並寫入 Limitations。輸入之一是 [`R0` regime probe](archive/R0_REGIME_PROBE_RECEIPT_2026-09-11.md)，已於 2026-09-11 執行完成（兩個對比皆 `R0_WINDOW_FOUND`）；其「regime 由 horizon 決定」的量測應寫入 §3 taxonomy 與 A-C3 論述。
3. `PUB-B0` 授權已取得（2026-09-10）。剩餘依序：專案負責人決定 [PUBLICATION_PLAN §5](PUBLICATION_PLAN.md) 的兩個子問題（規則、FORMAL 範圍花在哪條線）→ `PUB-B4` OSF preregistration（只有負責人能做）→ `SELECT-AMENDMENT-01`（`EP-03` narrowing amendment 並重新 pin digest）→ `EP-01`／`EP-02` amendment。在子問題未決前不鑄造 authorization evidence、不解封 `20000–20029`。 **2026-09-22 更新：** 子問題已決定——**延後**，seeds 保持封存（[decision](TRACK_B_FORMAL_EVALUATION_DECISION_2026-09-22.md)）；此列其餘步驟為解除延後後的順序。

**工程（見 [ROADMAP](ROADMAP.md) §9）**

1. [DONE 2026-09-14] [`TRACKED-LINEAGE-TRAINING-V1`](TRACKED_LINEAGE_TRAINING_SPEC.md) 已執行完成（[receipt](archive/TRACKED_LINEAGE_TRAINING_RECEIPT_2026-09-14.md)）。**下一個決定是 receipt §10 的四條路線之一**（提高 budget／引入 curriculum／改 reward 或環境／改以其他方式解 `PUB-B3`），全部需要**新的 protocol 版本**並揭露它是在已知本結果的情況下設計的；`2,000,000` 步上限不得事後上調，20 個已保留 checkpoint 不得刪除。
...
`backend/`：**1 failed / 1135 passed**（**2026-09-22 重新量測**，`517.19` s，1,136 收集，`b4b5900` 的乾淨工作樹（凍結 commit；其後的 `16b9275` 只有文件、無程式變更）；比同日 `c4cd809` 的 `1,114` 多 **22**，全部來自任務 #107 的 `test_v1_actuated_energy_suite.py`；失敗項仍是同一個、未放寬的 `PRIMARY_CASE_RECEIPT_IDENTITY`）。前一次：**1 failed / 1113 passed**（**2026-09-22**，`550.82` s，1,114 收集，`c4cd809` 的乾淨工作樹（凍結 commit；其後的 `59eab53` 只有文件、無程式變更）；比同日 `64ad5d6` 的 `1,092` 多 **22**，全部來自任務 #106 的 `test_v1_contact_reference_suite.py`）。再前一次：**1 failed / 1091 passed**（**2026-09-22**，`651.94` s，1,092 收集，`64ad5d6` 的內容——run 在該 commit 建立前、於內容相同的工作樹啟動；對 `4a96f54` 只多 V1 receipt 與文件對齊、無程式變更；比同日 `a743db9` 的 `1,072` 多 **20**，全部來自任務 #105 的 `test_v1_dynamic_reference_suite.py`）。再前一次：**1 failed / 1071 passed**（**2026-09-22**，`420.57` s，1,072 收集，`a743db9`；同日稍早 `0dca910` 為 `430.68` s 同數——`a743db9` 沒有新增 backend 測試，6 項瀏覽器測試在 `frontend/e2e`、刻意不在此集合；比 2026-09-20 的 `1061` 多 **10**，全部來自任務 #102 的 `test_controller_cp.py`——以逐檔 collect 對 `main`（`2a2a9f8`）比對確認只有這一檔不同）。再前一次：**1 failed / 1061 passed**（**2026-09-20**，`499.18` s，`492f452`；量測時同機併跑 API 與前端 dev server，故耗時長於先前的 `408.43` s）。完整報告——含三個契約、796 個連結與五個畫面的實際截圖與數據——見 [TEST_REPORT_2026-09-20](TEST_REPORT_2026-09-20.md)。逐步對帳如下，起點為 **1 failed / 1034 passed**（2026-09-17，`577.73` s，工作樹為 `1ec13c3` 加上當時的變更，`python3 -X utf8 -m pytest backend/ -p no:cacheprovider -q --durations=0`）。數字逐步對得起來：2026-09-14 的 `941` ＋7（介面改版的兩支測試檔：`test_warning_items.py` 5 個、`test_decision_kind.py` 2 個）＝ `948`（`9f871ac`）；＋2（PR #25 補上的 `record_start` 時長邊界與預設值回歸測試，均在 `test_run_trace.py`）＝ `950`（`979e73b`）；＋27（`GATE-STATUS-SINGLE-SOURCE-V1` 的 `test_gate_status_contract.py`）＝ `977`（`01afa60`）；＋26（`DERIVED-CLAIM-CONSISTENCY-V1` 的 `test_derived_claim_contract.py`）＝ `1003`（`5bfc342`，乾淨樹實測）；＋25（`TEACHING-BOUNDARY-V1` 的 `test_teaching_boundary_contract.py`）＝ `1028`（`1ec13c3`）；**−25 ＋31**（該檔更名為 `test_module_boundary_contract.py`，25 個保留、6 個為工具組邊界新增）＝ `1034`（`8cca46c`）；**＋8**（`RUN-MANIFEST-LOCK-BINDING-V1` 分析期 gate 的 `LB-08` 路徑逃逸正控制，含兩個負控制）＝ `1042`（`5c09722`）；**＋11**（任務 #95：兩個 gate 重算被綁定的 lock record，含兩個**多故障**順序控制與一個對真實保留證據跑的 blast-radius 測試）＝ `1053`（`5f74113`）；**＋3**（任務 #92：把 [TOOLKIT_USAGE](TOOLKIT_USAGE.md) 的 vendoring 宣稱變成被檢查的事實）＝ `1056`（2026-09-19，`410.17` s）；**＋5**（任務 #93：三個守相對 import 與平坦 vendoring 的測試，加兩個守 `learning_fingerprint` 副作用 docstring 的雙向測試）＝ **`1061`**（2026-09-19，`400.57` s）。**＋0**（任務 #96：產品 B 搬進 `backend/toolkit/`——純搬移，測試數不變）＝ `1061`（2026-09-19，`407.95` s）；**＋0**（任務 #97：§4.2 封存——22 個檔案進 `backend/archive/`，測試**繼續跑**因此數目不變）＝ **`1061`**（2026-09-19，`408.43` s）；**＋10**（任務 #102：Capture-point 對照組的 `test_controller_cp.py`——hook 預設值等於舊公式、DCM 解析式、公開指令拒絕 `cp`／內部切換成功）＝ **`1071`**（2026-09-22，`430.68` s，`0dca910`）；**＋20**（任務 #105：V1 動態參考案例的 `test_v1_dynamic_reference_suite.py`——凍結契約一致、橢圓積分已知值、四個 MJCF 變換各恰一次、閉式慣量、primary PASS、stdlib replay 相符、六種結構篡改 raise）＝ **`1092`**（2026-09-22，`651.94` s，`64ad5d6` 的內容——run 在該 commit 建立前、於內容相同的工作樹啟動；對 `4a96f54` 只多 V1 receipt 與文件對齊、無程式變更）；**＋22**（任務 #106：V1 接觸參考案例的 `test_v1_contact_reference_suite.py`——凍結契約一致、閉式對引擎模型語義、離散觸地步、plant 地板 MJCF、primary PASS、假說不改 suite 狀態、replay 相符、兩種有限值篡改、八種結構篡改）＝ **`1114`**（2026-09-22，`550.82` s，`c4cd809` 的乾淨工作樹（凍結 commit；其後的 `59eab53` 只有文件、無程式變更））；**＋22**（任務 #107：V1 致動能量帳的 `test_v1_actuated_energy_suite.py`——凍結契約一致、正弦命令在 forcerange 內、MJCF 變換恰好一次且保留致動器、蹲起參考等於 plant 站立律、primary PASS、replay 相符、扭矩與接觸力篡改、九種結構篡改）＝ **`1136`**（2026-09-22，`517.19` s，`b4b5900` 的乾淨工作樹（凍結 commit；其後的 `16b9275` 只有文件、無程式變更））。失敗項仍是同一個、未放寬的 reduction-order 差異，無新增失敗。

[BLOCKER] **牆鐘時間不可跨次比較，這次差了 1.80 倍。** `577.73` s 對上前一次的 `321.59` s，同一個容器、不同負載；`--durations=0` 顯示沒有任何單一測試主導這個差距。**因此本列記錄的是量到的秒數，不做正規化**，而秒數**不**被當成回歸指標——會被當成回歸指標的是通過數與失敗身分。

[RESULT] **三個文件契約於同一工作樹複跑通過**：`GATE_STATUS_SINGLE_SOURCE_CONSISTENT`（33 gates／54 sites）、`DERIVED_CLAIMS_CONSISTENT`（1 claim／3 sites／10 papers）、`MODULE_BOUNDARIES_CLEAN`（teaching 15 模組／4,943 行，toolkit 7 模組／5,300 行）。全 repo 的相對 markdown 連結全部可解析。
...
[RESULT] **失敗身分已直接核對，不只比對測試名**：重跑 replay 取出失敗判準為 `PRIMARY_CASE_RECEIPT_IDENTITY`（`replay["status"] == "FAIL"`），與 [V2 receipt §8](archive/TRACKED_LINEAGE_TRAINING_V2_RECEIPT_2026-09-14.md) 記載的同一個 reduction-order 差異同源。另於 2026-09-17 以 `git stash` 在合併基底上重跑確認**該失敗先於這一系列變更存在**。
